	...
	public function _customEvent_whenThisHearstrigger():Void
	{
		if(_Fired)
		{
			if(!(_BulletCreated.isAlive()))
			{
				...
				{
					if(((Engine.engine.getGameAttribute("PierceFire") : Float) == 0))
					{
					...
					else
					{
						...
						Engine.engine.setGameAttribute("PierceFire", ((Engine.engine.getGameAttribute("PierceFire") : Float) - 1));
					}
				}
				...
				{
					getLastCreatedActor().setVelocity((((Utils.DEG * actor.getAngle()) + shooterangle) - 180), (speed + 15));
					playSound(getSound(46));
				}
				else if(((Engine.engine.getGameAttribute("BulletNo") : Float) == 3))
				{
					getLastCreatedActor().setVelocity((((Utils.DEG * actor.getAngle()) + shooterangle) - 180), (speed - 6));
					playSound(getSound(45));
				}
				else if(((Engine.engine.getGameAttribute("BulletNo") : Float) == 4))
				{
					getLastCreatedActor().setVelocity((((Utils.DEG * actor.getAngle()) + shooterangle) - 180), (speed - 2));
